...
| ~~NOTIFY Handler Goroutine Leak~~ **FIXED** | ~~Goroutine captures s.lifecycleCtx which was context.Background()~~ Now properly checks ctx.Done() and s.done in select | ~~Goroutines leak on shutdown~~ Fixed in PR #68 |
...
#### 4. Dynamic Update Transaction Failure — FIXED
...
| **~~Recursive resolution has no total timeout~~ FIXED** | ~~Up to 225s blocked per root server failure~~ Timeout now enforced in fallback loop | ~~225s blocking under upstream failures~~ Fixed in PR #69 |
...
#### Recursive Resolution No Total Timeout — FIXED
**File:** `recursive.go` (lines 55-175)

**Fixed in PR #69** by adding timeout checks in the fallback loop:

```go
// Before: fallback loop had no timeout check
for _, fallback := range resolver.fallbacks {
    resp, err := s.sendQueryInternal(serverAddr, name, qType, true)
}

// After: timeout check before each fallback
for _, fallback := range resolver.fallbacks {
...
**[x] High Risk** — Significant production concerns (reduced by 5 in PRs #58-59, #68-69):
- ~~AXFR has no authentication (critical security vulnerability)~~ — Fixed in PR #59
- ~~AXFR/IXFR memory explosion~~ — Fixed in PR #59
- ~~NOTIFY handler goroutine leak~~ — Fixed in PR #68
...
- ~~Recursive resolution can block for minutes under upstream failures~~ — Fixed in PR #69
