...
type DNSRecord struct {
	...
	Data  []byte
	IP    net.IP // A/AAAA
	Host  string // NS/CNAME/PTR/MD/MF/MB/MG/MR/MX/SRV
	Txt   string // TXT
	// MX/SRV
	Priority uint16
...
func (r *DNSRecord) Read(buffer *BytePacketBuffer) error {
	...
	case SRV:
		...
		r.Weight, err = buffer.Readu16()
		if err != nil {
			return err
		}
		r.Port, err = buffer.Readu16()
		if err != nil {
			return err
		}
		r.Host, err = buffer.ReadName()
...
func (r *DNSRecord) Write(buffer *BytePacketBuffer) (int, error) {
	...
	case SRV:
		...
		buffer.Writeu16(0)
		buffer.Writeu16(r.Priority)
		buffer.Writeu16(r.Weight)
		buffer.Writeu16(r.Port)
		buffer.WriteName(r.Host)
		currPos := buffer.Position()
		buffer.Seek(lenPos)
		buffer.Writeu16(uint16(currPos - (lenPos + 2)))
