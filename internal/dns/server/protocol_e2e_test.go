...
func TestEndToEnd_RFC_Extensions(t *testing.T) {
	...
	// Get the starting serial (from the default SOA created by CreateZone)
	...
	// 5. Verify change via API (Ensure we use the 'admin' tenant from zone creation)
	resp, _ = http.Get(fmt.Sprintf("http://%s/zones/%s/records?tenant_id=admin", apiAddr, createdZone.ID))
	...
	if !foundDynamic { t.Errorf("Dynamic record not found via API after update. Records: %v", zoneRecs) }
	...
	// Current SOA in Authority section (per RFC 1995)
	...
	// Read IXFR stream (Simplified: just check first few packets)
	// Response should be: Current SOA, then diffs...
	lenB := make([]byte, 2)
	tcpConn.Read(lenB)
	ixfrRLen := uint16(lenB[0])<<8 | uint16(lenB[1])
	ixfrRData := make([]byte, ixfrRLen)
	tcpConn.Read(ixfrRData)
	...
	if len(resIXFR.Answers) == 0 || resIXFR.Answers[0].Type != packet.SOA {
		t.Errorf("IXFR failed to start with SOA")
	}
	if resIXFR.Answers[0].Serial != startSerial+1 {
